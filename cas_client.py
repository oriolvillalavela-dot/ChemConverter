...
def _pick_name(rec):
    """Prefer IUPAC/preferred, then synonyms, strip HTML/markup."""
    for path in [
...
class RequestHandler:
    ...
    def _mask_bearer(self, headers: dict) -> dict:
        ...
        if "Authorization" in masked:
            masked["Authorization"] = re.sub(r"(Bearer\s+)([A-Za-z0-9\-_\.]+)", r"\1***REDACTED***", masked["Authorization"]
            )
        return masked
    ...
    def post(self, url: str, json_body: Optional[dict] = None) -> dict:
        ...
        snap = {
            "request": {"method": "POST", "url": url, "json": json_body or {}, "headers": self._mask_bearer(dict(self.session.headers))},
    ...
    def get(self, url: str, params: Optional[dict] = None) -> dict:
        ...
        snap = {
            "request": {"method": "GET", "url": url, "params": params or {}, "headers": self._mask_bearer(dict(self.session.headers))},
            "status_code": r.status_code,
            "text": r.text[:4000],
        }
        try:
...
class CASClient:
    ...
    def _extract_fields(self, details: dict, fallback_smiles: Optional[str] = None, include_full: bool = False, include_wide: bool = False, include_mf_mw: bool = False) -> Dict:
